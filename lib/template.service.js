...
const fontPathTimesBold = path.join(publicDir, "fonts", "timesbd.ttf");
const fontPathTahoma = path.join(publicDir, "fonts", "tahoma.ttf");

function getFontBase64() {
  try {
    const fontBufferTimes = fs.readFileSync(fontPathTimes);
    const fontBufferTimesBold = fs.readFileSync(fontPathTimesBold);
    const fontBufferTahoma = fs.readFileSync(fontPathTahoma);
    return {
      fontBufferTimes: fontBufferTimes.toString("base64"),
      fontBufferTimesBold: fontBufferTimesBold.toString("base64"),
...
exports.generateHTMLATT = (options = {}) => {
  ...
  // Build font-face CSS: Times regular (400) + Times bold (700) + Tahoma
  const fontFaceCSS = fontBase64 ? `
    @font-face {
      font-family: 'MomoSignature';
      font-style: normal;
      font-weight: 400;
      src: url(data:font/truetype;charset=utf-8;base64,${fontBase64.fontBufferTimes}) format('truetype');
      font-display: block;
    }
    @font-face {
      font-family: 'MomoSignature';
      font-style: normal;
      font-weight: 700;
      src: url(data:font/truetype;charset=utf-8;base64,${fontBase64.fontBufferTimesBold}) format('truetype');
      font-display: block;
    }
    @font-face {
      font-family: 'Tahoma';
